...
def display_data_content(df):
    ...
    # data description
    st.write("<h6 style = 'text-align : center ;'> Analysis of all numerical data </h6>",unsafe_allow_html=True)
    st.dataframe(df.describe())

    st.dataframe(df.isnull().any())


def display_upload_data_page():

    # Title of page
    st.write("<h1 style = 'text-align : center;'> Upload the data </h1> ",unsafe_allow_html=True)
    st.write("---")

    # File uploading
    training_data_file_name = st.file_uploader("**Upload the training data**",["csv"])
